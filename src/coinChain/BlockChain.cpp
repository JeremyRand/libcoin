...
void BlockChain::purge_depth(int purge_depth) {
    if (purge_depth+1 < _purge_depth)
        log_warn("Requested a purge_depth (Persistence setting) deeper than currently, please re-download the blockchain to enforce this!"); //first sync should not have this warning
        ...
        for (size_t in_idx = 0; in_idx < inputs.size(); ++in_idx) {
            ...
            if (!output.isNull()) {
                ...
                if (_validation_depth == 0) {
                    ...
                    if (coin.count < 0 && _tree.count() + coin.count + 1 < _chain.maturity(_tree.height()))
                        throw Error(cformat("Tried to spend immature (%d confirmations) coinbase: %s", _tree.count() + coin.count + 1, hash.toString()).text());
                }
                else {
